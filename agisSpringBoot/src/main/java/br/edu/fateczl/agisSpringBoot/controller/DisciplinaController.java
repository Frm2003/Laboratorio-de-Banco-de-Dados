...
import br.edu.fateczl.agisSpringBoot.persistence.ProfessorDao;
...
public class DisciplinaController {
	...
	public ModelAndView disciplinaGet(ModelMap model) {
		...
		try {
			...
			professores.addAll(pdao.listar());
	...
	public ModelAndView disciplinaPost(@RequestParam Map<String, String> param, ModelMap model) {
		...
		try {
			...
			case "Inserir":
				inserir(nome, Integer.parseInt(qtdAulas), Integer.parseInt(semestre), horario, Integer.parseInt(curso), Integer.parseInt(professor));
				break;
			case "Buscar":
			...
			case "Atualizar":
				if (!disciplinas.isEmpty()) {					
					atualizar(Integer.parseInt(cod), nome, Integer.parseInt(qtdAulas), Integer.parseInt(semestre), horario, Integer.parseInt(curso), Integer.parseInt(professor));
				}
				break;
			case "Deletar":
				if (!disciplinas.isEmpty()) {					
	...
	private void inserir(String nome, int qtdAulas, int semestre, String horario, int curso, int professor) throws ClassNotFoundException, SQLException {
	...
	private void atualizar(int cod, String nome, int qtdAulas, int semestre, String horario, int curso, int professor) throws ClassNotFoundException, SQLException {
		for (Disciplina d : disciplinas) {
			if (d.getCod() == cod) {
				d.setNome(nome);
				d.setQtdAulas(qtdAulas);

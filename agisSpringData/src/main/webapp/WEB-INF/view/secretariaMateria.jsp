<%@ taglib uri="http://java.sun.com/jsp/jstl/core" prefix="c" %>
<%@ page language="java" contentType="text/html; charset=ISO-8859-1"
	pageEncoding="ISO-8859-1"%>
<!DOCTYPE html>
<html>
<head>
	<meta charset="ISO-8859-1">
	<link rel="stylesheet" href='<c:url value="./resources/estilo/nav.css"/>'>
	<link rel="stylesheet" href='<c:url value="./resources/estilo/slide.css"/>'>
	<link rel="stylesheet" href='<c:url value="./resources/estilo/style.css"/>'>
	<script src='<c:url value="./resources/scripts/script.js"/>'></script>
	<title>Insert title here</title>
</head>
<body onload="load()">
	<nav>
		<ul class="alin-se m15v">
			<li><a href="index">HOME</a></li>
			<li><a href="aluno">Matricular Aluno</a></li>
			<li><a href="curso">Inserir Curso</a></li>
			<li><a href="materia">Inserir Materia</a></li>
			<li><a href="disciplina">Inserir Disciplina</a></li>
			<li><a href="historicoAluno">Historico Aluno</a></li>
		</ul>
	</nav>
	<section class="sec1">
		<div class="navigation">
			<label for="r1" class="bar labelTitulo p05" checked>Formulário</label> 
			<label for="r2" class="bar labelTitulo p05">Lista Completa</label> 
		</div>
		<div class="slides">
			<input type="radio" name="r" id="r1"> 
			<input type="radio" name="r" id="r2"> 
			<div class="content s1">
				<form class="alin-se" action="materia" method="post">
					<div>
						<div>
							<div class="campo em15v alin-se">
								<input class="caixaDeTexto m15v" value='<c:out value="${materia.cod }" />' name="cod" type="number" placeholder="cod"> 
								<input class="btSimples" type="submit" name="botao" value="Buscar">
							</div>
							<p>Inserção de dados</p>
							<div class="alin-se m15v">
								<select name="horario" class="caixaDeTexto">
									<c:choose>
									    <c:when test="${empty materia.horario }">
											<option>Selecione o horario</option>
									    </c:when>
									    <c:otherwise>
									      	<option><c:out value="${materia.horario }"></c:out></option>
									    </c:otherwise>
									</c:choose>
									
									<option value="13:00 a 16:30">13:00 a 16:30</option>
									<option value="13:00 a 14:40">13:00 a 14:40</option>
									<option value="14:50 a 18:20">14:50 a 18:20</option>
									<option value="14:50 a 16:30">14:50 a 16:30</option>
									<option value="16:30 a 18:20">16:30 a 18:20</option>
									
									<option value="19:20 a 22:50">19:20 a 22:50</option>
									<option value="19:20 a 20:40">19:20 a 20:40</option>
									<option value="20:40 a 22:50">20:40 a 22:50</option>
									<option value="20:40 a 22:50">20:40 a 22:50</option>
									<option value="08:00 a 11:30">08:00 a 11:30</option>
								</select> 
							</div>
							
							<div class="alin-se m15v">
								<select name="tipoAv" class="caixaDeTexto">
									<c:choose>
									    <c:when test="${empty materia.tipoAvalicao }">
											<option>Selecione o critério avaliativo</option>
									    </c:when>
									    <c:otherwise>
									      	<option><c:out value="${materia.tipoAvalicao }"></c:out></option>
									    </c:otherwise>
									</c:choose>
									
									<option value="Tipo1">Tipo 1</option>
									<option value="Tipo2">Tipo 2</option>
									<option value="Tipo3">Tipo 3</option>
									<option value="Tipo4">Tipo 4</option>
								</select> 
							</div>
							
							<div class="alin-se m15v">
								<select name="diaDaSemana" class="caixaDeTexto">
									<c:choose>
									    <c:when test="${empty materia.diaDaSemana }">
											<option>Selecione o dia da semana</option>
									    </c:when>
									    <c:otherwise>
									      	<option><c:out value="${materia.diaDaSemana }"></c:out></option>
									    </c:otherwise>
									</c:choose>
									<option value="Segunda">Segunda</option>
									<option value="Terca">Terça</option>
									<option value="Quarta">Quarta</option>
									<option value="Quinta">Quinta</option>
									<option value="Sexta">Sexta</option>
									<option value="Sabado">Sábado</option>
								</select> 
							</div>
							<div class="alin-se m15v">
								<select name="codDisci" class="caixaDeTexto">
									<c:choose>
									    <c:when test="${empty materia.disciplina.nome }">
											<option>Selecione a disciplina</option>
									    </c:when>
									    <c:otherwise>
									      	<option><c:out value="${materia.disciplina.nome } - ${materia.disciplina.curso.turno }"></c:out></option>
									    </c:otherwise>
									</c:choose>
									<c:forEach var="c" items="${disciplinas }">
										<option value="${c.cod }">
											<c:out value="${c.nome } - ${c.curso.turno }" />
										</option>
									</c:forEach>
								</select> 
							</div>
							<div class="alin-se m15v">
								<select name="codProf" class="caixaDeTexto">
									<c:choose>
									    <c:when test="${empty materia.professor.nome }">
											<option>Selecione o professor</option>
									    </c:when>
									    <c:otherwise>
									      	<option value="${materia.professor.cod }"><c:out value="${materia.professor.nome }"></c:out></option>
									    </c:otherwise>
									</c:choose>
									<c:forEach var="c" items="${professores }">
										<option value="${c.cod }">
											<c:out value="${c.nome }" />
										</option>
									</c:forEach>
								</select> 
							</div>
						</div>
						<div class="alin-se m15v">
							<input class="btSimples" type="submit" name="botao" value="Inserir">
							<input class="btSimples" type="submit" name="botao" value="Atualizar">
							<input class="btSimples" type="submit" name="botao" value="Deletar">
						</div>
					</div>
				</form>
			</div>
			<div class="content">
				<div class="alin-se w50p m15v">
					<div class="w50p">
						<div class="tabelaLinha m15v">
							<p class="colTitulo" id="col">Nome</p>
							<p class="colTitulo" id="col">Turno</p>
							<p class="colTitulo" id="col">Professor</p>
							<p class="colTitulo" id="col">Dia da Semana</p>
							<p class="colTitulo" id="col">Horario</p>
							<p class="colTitulo" id="col">Tipo Av.</p>
						</div>
						<c:forEach var="m" items="${materias }">
							<div class="tabelaLinha p05">
								<p id="col"><c:out value="${m.disciplina.nome }" /></p>
								<p id="col"><c:out value="${m.disciplina.curso.turno }" /></p>
								<p id="col"><c:out value="${m.professor.nome }" /></p>
								<p id="col"><c:out value="${m.diaDaSemana }" /></p>
								<p id="col"><c:out value="${m.horario }" /></p>
								<p id="col"><c:out value="${m.tipoAvalicao }" /></p>
							</div>
						</c:forEach>
					</div>
				</div>
			</div>
		</div>
	</section>
</body>
</html>